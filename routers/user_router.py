...
def get_user_by_username(username: Optional[str] = Query(min_length=4, max_length=25)):
  ...
  if not result_query:
    return JSONResponse(status_code=status.HTTP_400_BAD_REQUEST, content={"message": "not found"})
  return JSONResponse(status_code=status.HTTP_200_OK, content=jsonable_encoder(result_query))
...
def create_user(user: User) -> dict | JSONResponse:
  ...
  return JSONResponse(status_code=status.HTTP_201_CREATED, content={ "message": "Se ha registrado User" })
...
def delete_user(id: int):
  ...
  return JSONResponse(status_code=status.HTTP_200_OK, content={ "message": "Se ha eliminado User" })
